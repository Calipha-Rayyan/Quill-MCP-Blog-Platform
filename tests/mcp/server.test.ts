...
test("authenticated MCP endpoint exposes every tool and enforces ownership", async () => {
    ...
    const userA = auth.registerUser(
        "mcp-a@example.com",
        "secure password",
    );
    const userB = auth.registerUser(
        "mcp-b@example.com",
        "secure password",
    );
    ...
    const tool = async (
        ...
        const response = await rpc(key, "tools/call", {
            name,
            arguments: args,
        });
    ...
    try {
        const initialized = await rpc(keyA, "initialize", {
        ...
        assert.deepEqual(
            tools.map((item: ToolDefinition) => item.name),
            [
                "create_post",
                "update_post",
                "delete_post",
        ...
        for (const toolDefinition of tools) {
            ...
            assert.equal(
                JSON.stringify(toolDefinition.inputSchema).includes(
                    "user_id",
                ),
                false,
            );
        ...
        const created = await tool(keyA, "create_post", {
            title: "MCP post",
            content_md: "# MCP",
        });
        ...
        assert.equal(
            (await tool(keyA, "list_posts")).value.length,
            1,
        );

        assert.equal(
            (await tool(keyA, "get_post", { id: postId })).value.id,
            postId,
        );

        assert.equal(
            (await tool(keyA, "publish_post", { id: postId })).value.status,
            "published",
        );

        assert.ok(
            (await tool(keyA, "get_post", { id: postId })).value
                .published_at,
        );

        assert.equal(
            (await tool(keyA, "unpublish_post", { id: postId })).value
                .status,
            "draft",
        );

        const scheduled = await tool(keyA, "schedule_post", {
            id: postId,
            publish_at: new Date(
                Date.now() + 60_000,
            ).toISOString(),
        ...
        assert.equal(
            postService.getPublishedPostBySlug(
                scheduled.value.slug,
            ),
            null,
        );
        ...
        assert.equal(
            (await tool(keyB, "get_post", { id: postId })).isError,
            true,
        );

        assert.equal(
            (
                await tool(keyB, "update_post", {
                    id: postId,
                    title: "Stolen",
                })
            ).isError,
        ...
        assert.equal(
            (await tool(keyB, "delete_post", { id: postId })).isError,
            true,
        );
        ...
        assert.deepEqual(
            (await tool(keyB, "get_analytics")).value,
            [],
        );

        assert.deepEqual(
            (await tool(keyA, "get_analytics")).value,
            [{ post_id: postId, event_count: 1 }],
        );

        assert.deepEqual(
            (await tool(keyA, "delete_post", { id: postId })).value,
            { deleted: true },
        );
